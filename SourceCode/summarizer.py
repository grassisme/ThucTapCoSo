...
def combined_extractive(text: str, num_sentences: int = 3) -> str:
    ...
    try:
        ...
        return lsa_summary(text, num_sentences)
...
def voting_summary(text: str, num_sentences: int = 3) -> str:
    """
    Kết hợp LSA + TextRank + TextTeaser bằng cách voting:
    Câu được nhiều thuật toán chọn → điểm cao hơn → ưu tiên hơn.
    """
    _ensure_nltk()
    cleaned = clean_ocr_text(text)

    votes = {}  # sentence -> số lần được chọn
    ...
    try:
        ...
        for s in lsa(parser.document, num_sentences):
            votes[str(s)] = votes.get(str(s), 0) + 2  # LSA weight = 2

        tr = TextRankSummarizer(stemmer)
        tr.stop_words = stop_words
        for s in tr(parser.document, num_sentences):
            votes[str(s)] = votes.get(str(s), 0) + 2  # TextRank weight = 2
    except Exception:
        pass
    ...
    try:
        ...
        for s in tt.summarize(first, cleaned)[:num_sentences]:
            votes[s] = votes.get(s, 0) + 3  # TextTeaser weight = 3 (tốt hơn)
    except Exception:
        pass
    ...
    # Sắp xếp theo số vote, sau đó theo thứ tự xuất hiện
...
def summarize(
    ...
    print("[*] Đang voting LSA + TextRank + TextTeaser...")
    ...
    return {
        ...
        "textrank": tr,
        "textteaser": tt,
        "voting": voting,        # ← tốt nhất, dùng cái này
